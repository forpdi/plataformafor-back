...
public final class CoreMessages {
	...
	/**
	 * Retorna o texto no locale padrão (inglês).
	 *
	 * @param key
	 *            Chave do texto.
	...
	public static String getText(String key) {
		try {
			return ResourceBundle.getBundle(BUNDLE_NAME, DEFAULT_LOCALE).getString(key);
	...
	public String formatText(String key, Locale locale, Object[] args) {
		try {
			String raw = ResourceBundle.getBundle(BUNDLE_NAME, locale).getString(key);
	...
	/**
	 * Obtem uma mensagem dada a chave.
	...
	public String getMessage(String key) {
		try {
			...
			if (message == null) {
				throw new MissingResourceException("", ResourceBundle.class.getName(), key);
			}
			return message;
		} catch (MissingResourceException ex) {
			return "???"+key+"???";
