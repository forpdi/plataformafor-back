...
public class PlanBS extends HibernateBusiness {
	...
	@Autowired private OnLevelInstanceUpdateTask levelInstanceUpdateTask;
	...
	public List<Plan> listPlansByStructureId(long structureId) {
		Criteria criteria = this.dao.newCriteria(Plan.class)
			.add(Restrictions.eq("deleted", false))
			.add(Restrictions.eq("structure.id", structureId));
		return this.dao.findByCriteria(criteria, Plan.class);
	}
